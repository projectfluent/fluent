...
export const flatten = depth => list =>
    list.reduce(
        (acc, cur) =>
...
export const mutate = state => obj => Object.assign(obj, state);

// Join the list of parsed values into a string.
export const join = list => list.filter(value => value !== Symbol.for("eof")).join("");

// Prune unmatched maybes from a list.
export const prune = list => list.filter(value => value !== null);
...
export const print = x => (console.log(JSON.stringify(x, null, 4)), x);
